...
from hotel_prompt import hotel_scrape_task
...
class HotelSearchScraper:
    ...
    async def set_travelers(self, adults: int = 2):
        try:
            ...
            try:
                ...
                counter = await self.page.wait_for_selector('div[aria-label*="Adults"] span[jsname="NnAfwf"]', timeout=2000)
                if not counter:
                ...
                if adults > current_adults:
                    ...
                    for i in range(clicks_needed):
                        try:
                            add_button = await self.page.wait_for_selector('button[aria-label="Add adult"]', timeout=2000)
                            if add_button:
                    ...
                    for i in range(clicks_needed):
                        try:
                            remove_button = await self.page.wait_for_selector('button[aria-label="Remove adult"]', timeout=2000)
                            if remove_button:
    ...
    async def select_dates(self, checkin_date_str, checkout_date_str):
        try:
            ...
            date_button = await self.page.wait_for_selector('button[data-modal-type="dates"], [aria-label="Change dates"], [aria-label="Check-in"]', timeout=5000)
...
async def scrape_hotels(url):
    browser = Browser(config=BrowserConfig(headless=False))
    initial_actions = [{"open_tab": {"url": url}}]
    agent = Agent(task=hotel_scrape_task(url), llm=ChatOpenAI(model="gpt-4o-mini"), initial_actions=initial_actions, browser=browser)
...
async def get_hotel_url(location, checkin_date, checkout_date, adults):
    try:
        scraper = HotelSearchScraper()
        await scraper.start(use_bright_data=False)
        url = await scraper.search_hotels(location, checkin_date, checkout_date, adults)
        return url
    finally:
